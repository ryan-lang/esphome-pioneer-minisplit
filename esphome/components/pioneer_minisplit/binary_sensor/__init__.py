...
CONFIG_SCHEMA = (
    cv.Schema({
        ...
        cv.Optional(CONF_DEEP_SLEEP): binary_sensor.binary_sensor_schema(PioneerMinisplitBinarySensor, icon="mdi:sleep").extend(cv.COMPONENT_SCHEMA),
        cv.Optional(CONF_SLEEP): binary_sensor.binary_sensor_schema(PioneerMinisplitBinarySensor, icon="mdi:sleep").extend(cv.COMPONENT_SCHEMA),
        cv.Optional(CONF_ECO): binary_sensor.binary_sensor_schema(PioneerMinisplitBinarySensor, icon="mdi:sprout").extend(cv.COMPONENT_SCHEMA),
        cv.Optional(CONF_TURBO): binary_sensor.binary_sensor_schema(PioneerMinisplitBinarySensor, icon="mdi:car-turbocharger").extend(cv.COMPONENT_SCHEMA),
    })
)
